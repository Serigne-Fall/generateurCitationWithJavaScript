...
let divCitation=document.querySelector('.citation');
let divAuteur=document.querySelector('#auteur');
let hasard=0;
let dernier=0;
...
btn.addEventListener('click',()=>{
    do {
        hasard=genererEntier(citations.length);
        console.log(hasard)
    }while (hasard==dernier);

    let citation=citations[hasard][0];
    let auteur=citations[hasard][1];
    divCitation.textContent=citation;
    divAuteur.textContent=auteur;
    dernier=hasard;
